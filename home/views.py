...
def excel(list): # xuat tat ca
    ...
    headers = ["STT","Ngày mượn","Họ và tên người mượn","Tên thiết bị hoặc hóa chất được sử dụng", "Tên bài/Tiết dạy", "Số lượng","Tiết TKB","TIết PPCT","Lớp dạy","Chữ ký GV mượn","Ngày trả","Tình trạng trả","Chữ ký GV trả"] 
    ...
    for row_num, device in enumerate(list, start=2):
        ...
        worksheet.cell(row=row_num, column=3, value=device.userId.name)
        worksheet.cell(row=row_num, column=4, value=device.deviceId.name)
        worksheet.cell(row=row_num, column=5, value=device.tenbai)
        worksheet.cell(row=row_num, column=6, value=1)
        worksheet.cell(row=row_num, column=7, value=device.tiet)
        worksheet.cell(row=row_num, column=8, value=device.tietPPCT)
        worksheet.cell(row=row_num, column=9, value=device.lop)
        worksheet.cell(row=row_num, column=10, value=" ")
        worksheet.cell(row=row_num, column=11, value=device.ngaytra)
        worksheet.cell(row=row_num, column=12, value=device.deviceId.status)
...
def locCuaLoc(timKiem):
    sum =0
    if(timKiem['mon']!="" and timKiem['giaovien']=="" and timKiem['tu']=="" and timKiem['den']==""):
...
def getThongKe(request):
    ...
    if checkLogin(request): 
        ...
        if request.method == 'POST':
            ...
            timKiem = {
                'mon': request.session.get('mon'),
                'giaovien': request.session.get('giaovien'),
                'tu': request.session.get('tu'),
                'den':request.session.get('den')
            }
            ...
            timKiem['tu']=ngays
            timKiem['den']=ngaye
            timKiem['giaovien']=giaovien
            request.session['mon'] = mon
            request.session['tu'] = ngays
            request.session['den'] = ngaye
            request.session['giaovien'] = giaovien
            data =locCuaLoc(timKiem)
            if action == "loc": # thống kê theo lọc
                listT =thongBao(request)
                return render(request, 'pages/thongke.html',{"device":giosangtiet(data[0]), "sum":data[1],"name":nameUser,"role":rl,"thongbao":listT,"id":id,"mon":timKiem['mon'], "tu":timKiem['tu'], "den":timKiem['den'], "giaovien":timKiem['giaovien']})
            if action == "lvx": 
                datal =locCuaLoc(timKiem)
                file = xuatDanhSachTheoLoc(giosangtiet(datal[0]))
                return file
            if e != None: 
